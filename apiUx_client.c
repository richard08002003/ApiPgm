    ...
    while(1) {
        ...
        if (fpid < 0 ) {
            ...
            **send() 寫資料*/
            while(1) {
                while(1) {
                    ...
                    fp4 = fopen( "/home/richard/Richard/air_main/20150114/TestLog/Api_SendLog.txt" , "w" ) ;
            ...
            **recv() 讀取資料*/
                while(1) {
                    char str[1000] = {0} ;
                    fp5 = fopen( "/home/richard/Richard/air_main/20150114/TestLog/Api_ReceiveLog.txt" , "w" ) ;
                    if ( fp5 == NULL) {
                        printf("Api_ReceiveLog.txt 檔案開啟錯誤 ! ") ;
                        break ;
                    }
                    ...
                    if ( val < 0 ) {
                        ...
                        fprintf( fp5 , "%s " , "\nDisconnected to Maingateway server . . . \n" ) ;
                        break ;
                    } else {
                        printf("\n接收 MainGateway Server's 的資料為：%s \n" , str ) ;
                        fprintf( fp5 , "%s" , "\n接收 MainGateway Server's 的資料為：" ) ;
                        fprintf( fp5 , "%s\n" , str ) ;
                        // 將收到的資料壓成MD5
                        MD5_CTX ctx ;
                        int check = MD5_Init(&ctx) ;
